...
type TodoLoaderConfig struct {
	...
	Fetch func(keys []string) ([][]*model.Todo, []error)
...
func (l *TodoLoader) Load(key string) ([]*model.Todo, error) {
...
func (l *TodoLoader) LoadThunk(key string) func() ([]*model.Todo, error) {
	l.mu.Lock()
...
func (l *TodoLoader) LoadAll(keys []string) ([][]*model.Todo, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TodoLoader) LoadAllThunk(keys []string) func() ([][]*model.Todo, []error) {
	results := make([]func() ([]*model.Todo, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *TodoLoader) Prime(key string, value []*model.Todo) bool {
	l.mu.Lock()
	var found bool
...
func (l *TodoLoader) Clear(key string) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *TodoLoader) unsafeSet(key string, value []*model.Todo) {
	if l.cache == nil {
		l.cache = map[string][]*model.Todo{}
	}
	l.cache[key] = value
}
...
func (b *todoLoaderBatch) keyIndex(l *TodoLoader, key string) int {
